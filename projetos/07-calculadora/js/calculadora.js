...
const atualizarDisplay = function (texto) {
    if (novoNumero) {
...
const selecionarOperador = function (evento) {
    if (!novoNumero) {
        calcular();
        novoNumero = true;
...
const mapearTeclado = (evento) => {
    ...
    const teclaPermitida = () => Object.keys(mapaDoTeclado).indexOf(tecla) !== -1;
    if (teclaPermitida()) document.getElementById(mapaDoTeclado[tecla]).click();
};
